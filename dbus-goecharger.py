...
class DbusGoeChargerService:
  def __init__(self, servicename, paths, goechargernum, productname='go-eCharger', connection='go-eCharger HTTP JSON service'):
    self.goechargernum = goechargernum
    self.config = self._getConfig()
    deviceinstance = int(self.config['ONPREMISE']['Deviceinstance{0}'.format(self.goechargernum)])

    logging.info("registering VeDbusService {0} as instance {1} for charger {2}/{3}".format(servicename,deviceinstance , self.goechargernum,  goechargernum ))
    logging.info("{}.http_{:02d}".format(servicename, deviceinstance ))
    self._dbusservice = VeDbusService("{}.http_id{:02d}".format(servicename, deviceinstance ),dbusconnection())
    self._paths = paths
    ...
    self._dbusservice.add_path('/ProductId', 0xAAAA + goechargernum) #
    self._dbusservice.add_path('/ProductName', productname)
    self._dbusservice.add_path('/CustomName', self.config['ONPREMISE']['Name{0}'.format(self.goechargernum)])
  ...
  def _getConfig(self):
    ...
    # logging.info("getting config config{0}.ini".format(configno))
  ...
  def _getGoeChargerStatusUrl(self):
    ...
    if accessType == 'OnPremise':
        URL = "http://%s/status" % (config['ONPREMISE']['Host{0}'.format(self.goechargernum)])
    else:
  ...
  def _getGoeChargerMqttPayloadUrl(self, parameter, value):
    ...
    if accessType == 'OnPremise':
        URL = "http://%s/mqtt?payload=%s=%s" % (config['ONPREMISE']['Host{0}'.format(self.goechargernum)], parameter, value)
    else:
        raise ValueError("AccessType %s is not supported" % (config['DEFAULT']['AccessType']))
...
def main():
  ...
  logging.basicConfig(      format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                            ...
                            level=logging.INFO,
                            handlers=[
                                RotatingFileHandler('%s/dbus-goecharger.log' % (os.path.dirname(os.path.realpath(__file__))), maxBytes=100000, backupCount=10),
  ...
  try:
      ...
      for x in range(3):
          logging.info("starting service {0}".format(str(x)))

          #start our main-service
          pvac_output[str(x)] = DbusGoeChargerService(
